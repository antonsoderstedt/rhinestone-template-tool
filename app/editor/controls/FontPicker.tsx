    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        <button
          ...
          className="flex w-full items-center justify-between rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-3 text-left text-sm text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
        >
          <div className="min-w-0">
            <p className="font-medium">{selectedFont.displayName}</p>
            <p className="mt-1 truncate text-xs text-zinc-500" style={{ fontFamily: selectedFont.previewFontFamily }}>
              {safePreviewText}
            </p>
          </div>
        ...
        {open && (
          <ul
            ...
            className="absolute left-0 right-0 top-[calc(100%+8px)] z-20 max-h-80 overflow-y-auto rounded-2xl border border-zinc-800 bg-zinc-950 p-2 shadow-2xl"
          >
              ...
              return (
                <li key={font.fontId}>
                  <button
                    ...
                    className={`flex w-full items-start justify-between rounded-xl px-3 py-3 text-left transition focus:outline-none focus:ring-2 focus:ring-purple-500 ${selected ? 'bg-purple-500/15 text-white' : 'text-zinc-300 hover:bg-zinc-900'}`}
                  >
                    ...
                    {selected ? <Check className="mt-0.5 h-4 w-4 shrink-0 text-purple-300" /> : <Type className="mt-0.5 h-4 w-4 shrink-0 text-zinc-600" />}
                  </button>
                </li>
              );
            })}
